...
<AccordionGroup>
  ...
  <Accordion title="Local (GGUF + node-llama-cpp)">
    ...
    | `local.gpu`           | `"auto" \| "metal" \| "cpu"` | `"auto"` | GPU backend policy for local embeddings. `"auto"` keeps node-llama-cpp defaults, `"metal"` requests the Metal backend on supported Macs, and `"cpu"` disables GPU offload where the installed build supports it. CPU mode can be a safer Apple Silicon mitigation when Metal cleanup is unstable, but may be slower. |
